...
// 定义弹出层的变量
const showHositotal = ref(false);

const showHositotalcolumns = computed(() => {
...
const submit = async () => {
  ...
  for (const i of params) {
    if (!form[i]) {
      showNotify({ message: "请注意填写每一项" });
      return;
    }
  }

  const { data: orderData } = await proxy.$api.createOrder(form);

  // console.log(orderData.data.wx_code);
  qrcode.toDataURL(orderData.data.wx_code).then((url) => {
    showCode.value = true;
    codeImg.value = url;
  });
};
  ...
  <div class="container">
    ...
    <van-cell-group title="服务需求" class="cell">
      <van-field
        ...
        style="height: 100px"
        v-model="form.demand"
        placeholder="请简单描述下您要就诊的科室…"
      />
    </van-cell-group>
    <van-button type="primary" class="sumbit" @click="submit" size="large"
      >提交订单</van-button
    >
    <van-popup v-model:show="showHositotal" position="bottom" :style="{ height: '30%' }">
      <van-picker
        :columns="showHositotalcolumns"
        @confirm="onConfirm"
        @cancel="showHositotal = false"
      />
    </van-popup>
